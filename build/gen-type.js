...
fs.copyFileSync(
	...
	path.resolve(__dirname, '../lib/erui.d.ts'),
)

// index.d.ts
const newIndexPath = path.resolve(__dirname, '../lib/index.d.ts')
fs.copyFileSync(path.resolve(__dirname, '../lib/erui/index.d.ts'), newIndexPath)
const index = fs.readFileSync(newIndexPath)
const newIndex = index.toString().replace(/@erui\//g, './er-')
fs.writeFileSync(newIndexPath, newIndex)

// remove er
fs.rmdirSync(path.resolve(__dirname, '../lib/erui'), { recursive: true })
	...
	if (!noErPrefixFile.test(comp)) {
		if (fs.lstatSync(path.resolve(libDirPath, comp)).isDirectory()) {
			...
			const imp = fs
				...
				.toString()
			if (outsideImport.test(imp) || imp.includes('@erui/')) {
				const newImp = imp
					.replace(outsideImport, (i, c) => {
						return i.replace(`../${c}`, `../er-${c}`)
					})
					.replace('@erui/', '../er-')
				fs.writeFileSync(
					path.resolve(
	...
	if (fs.existsSync(srcPath)) {
		if (fs.lstatSync(srcPath).isDirectory()) {
			fs.readdir(srcPath, 'utf-8', (err, data) => {
				if (err) return
				// replace all @erui in src/*.d.ts
				data.forEach(f => {
					if (!fs.lstatSync(path.resolve(srcPath, f)).isDirectory()) {
						const imp = fs
							.readFileSync(path.resolve(srcPath, f))
							.toString()
						if (imp.includes('@erui/')) {
							const newImp = imp.replace(/@erui\//g, '../../er-')
